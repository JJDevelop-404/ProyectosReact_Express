...
import { nuevoPedido } from '../../API/Pedidos';
...
export default function Pedido() {
  ...
  const dataParaPedido = location.state ? location.state : null;
  ...
  const [lstIdProductos, setLstIdProductos] = useState([]);
  ...
  const onHandleClickOrdenar = () => {
    if (lstIdProductos.length === 0) {
      ...
      nuevoPedido(dataParaPedido.MesaId, MeseroId, lstProductos)
        .then((response) => {

          response ? alertaCrearEditar('Pedido Agregado', 'success', () => navigate(`/Mesero/${MeseroId}`))
            : alertError('Error al agregar el pedido', 'Intente de nuevo');
        })
    }
  }

  return dataParaPedido && dataParaPedido.MesaId && MeseroId ? (
      ...
      <>
        ...
        <div className="container-info-pedido">
          ...
          <p className='align-middle informacion-pedido'> Mesa N° {dataParaPedido && dataParaPedido.MesaId} </p>
